...
class RNN_model:
    ...
    def build_model(self, model_type='SimpleRNN', seq_len=5):
        ...
        model.add(tf.keras.layers.Conv1D(seq_len, kernel_size=3, strides=1, activation=tf.nn.relu, padding='same'))
...
for stock in STOCK:
    ...
    RNN.train(X_train, y_train, type=model_type[1])  # change Model yourself
