...
import os
import time
# you need to change the stock and model yourself, since there exist a problem when using for loop on deep learning
...
class RNN_model:
    ...
    def build_model(self, model_type='SimpleRNN', seq_len=5):
        ...
        if model_type == 'SimpleRNN':
            model.add(tf.keras.layers.Bidirectional(tf.keras.layers.SimpleRNN(seq_len, activation=tf.nn.relu)))
        elif model_type == 'LSTM':
            model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(seq_len, activation=tf.nn.relu)))
        elif model_type == 'GRU':
            model.add(tf.keras.layers.Bidirectional(tf.keras.layers.GRU(seq_len, activation=tf.nn.relu)))
